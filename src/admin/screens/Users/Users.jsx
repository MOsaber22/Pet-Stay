...
const INFOS = [
  ...
  },
  {
    ...
    bg: "bg-blue-gray-50",
    iconBg: "bg-blue-gray-100",
    iconColor: "text-blue-gray-500",
  },
];
      ...
      <div className="w-full px-4 sm:px-6 lg:px-2 py-6 flex flex-col gap-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <Typography
              variant="h1"
              className="font-extrabold  text-blue-gray-900 tracking-tight"
            >
              Platform Users
            </Typography>
            <Typography className="text-sm text-blue-gray-400 mt-1 max-w-xs leading-relaxed">
              Manage user roles, monitor account statuses, and oversee the
              Kindred Tails community membership.
            </Typography>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 primary-bg hover:bg-teal-800 text-white font-semibold text-md rounded-full px-5 py-3 transition-colors"
            >
              + Invite New User
            </button>
          </div>
        ...
        {invite ? (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="w-96 bg-white p-6 rounded-2xl shadow-xl flex flex-col gap-4">
              ...
              <Typography className="text-xs text-blue-gray-400 -mt-2">
                Send an invitation link to join the platform.
              </Typography>
              <input
                value={email}
              ...
              {message ? (
                <p
                  className={`text-xs ${message.includes("success") ? "text-teal-600" : "text-red-400"}`}
                >
                  {message}
                </p>
              ) : null}
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 border border-blue-gray-200 rounded-xl py-2.5 text-sm font-medium text-blue-gray-600 hover:bg-blue-gray-50 transition"
                >
                  Cancel
                </button>
                <button
                  onClick={handleInvite}
                  className="flex-1 bg-[#1C4D3A] hover:bg-[#163829] text-white rounded-xl py-2.5 text-sm font-semibold transition"
                >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[195px_1fr] gap-4">
          <Card className="bg-[#C6E8C3] shadow-none border-0 relative overflow-hidden">
            <CardBody className="p-2">
              ...
              <Typography className="text-sm font-semibold text-[#2D6B50]">
                Active Adopters
              </Typography>
              <Typography className="text-4xl font-extrabold text-[#1A3D2A] leading-none mt-1">
                {activeCount}
              </Typography>
              <Typography className="text-[10px] font-bold text-[#3A7D5C] uppercase tracking-wider mt-2">
                +12% this month
              </Typography>
            ...
            <CardBody className="p-5">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <Typography className="text-xs text-blue-gray-400 mt-0.5">
                    New registrations across all sectors
                  </Typography>
                </div>
              ...
              <div className="flex items-end gap-[5px] h-16 mt-4">
                {BARS.map((h, i) => (
                  <div
                    key={i}
                    className={`flex-1 rounded-sm ${i >= 4 ? "bg-[#2A8C69]" : "bg-blue-gray-100"}`}
                    style={{ height: `${h}%` }}
                  />
                ))}
              </div>
        ...
        <Card className="shadow-none border border-blue-gray-50 overflow-hidden">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="px-6 py-10 text-center text-blue-gray-400 text-sm"
                    >
                      User not found
                    </td>
                  </tr>
                  ...
                  visible.map(
                    ({ img, name, email, role, owner, active, date }, i) => (
                      <tr
                        key={`${name}-${i}`}
                        className={`hover:bg-blue-gray-50/40 transition-colors ${i < visible.length - 1 ? "border-b border-blue-gray-50" : ""}`}
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span
                              className={`h-2 w-2 rounded-full ${active ? "bg-teal-400" : "bg-blue-gray-300"}`}
                            />
                            <Typography
                              className={`text-sm ${active ? "text-blue-gray-700" : "text-blue-gray-400"}`}
                            >
          ...
          <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 px-6 py-3">
            <Typography className="text-[10px] font-bold uppercase tracking-widest text-blue-gray-400">
              Showing {visible.length} of {filtered.length} — Page {page} of{" "}
              {total}
            </Typography>
            <div className="flex gap-2">
              <IconButton
                variant="outlined"
                size="sm"
                className="rounded-full border-blue-gray-200 active:scale-95 transition-all"
              ...
              <IconButton
                size="sm"
                className="rounded-full bg-[#1C4D3A] shadow-none hover:bg-[#163829] active:scale-95 transition-all"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {INFOS.map(({ Icon, title, desc, bg, iconBg, iconColor }) => (
            <div
              key={title}
              className={`${bg} rounded-2xl p-4 flex items-start gap-3`}
            >
              <div
                className={`${iconBg} ${iconColor} rounded-xl p-2 shrink-0 mt-0.5`}
              >
              ...
              <div>
                ...
                <Typography className="text-xs text-blue-gray-500 mt-1 leading-relaxed">
                  {desc}
                </Typography>
              </div>
            </div>
