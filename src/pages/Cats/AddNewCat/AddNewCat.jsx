...
const AddNewCat = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${url}/cats`)
      .then((res) => res.json())
      .then(() => console.log("Backend check successful"))
      .catch((err) => console.warn("Backend check warning:", err.message));
  }, []);
  ...
  const validateForm = () => {
    ...
    if (!introduction.trim()) {
      ...
      tempErrors.introduction =
        "Please write a slightly longer story (min 15 characters).";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let numericAge = 3;
      if (age === "Kitten (0-1 year)") {
        numericAge = 1;
      } else if (age === "Adult (3-5 years)") {
        numericAge = 4;
      } else if (age === "Senior (8+ years)") {
        numericAge = 9;
      }
      data.append("age", numericAge);
      data.append("gender", "male");
      data.append("location", "Cairo");
      data.append("weight", 4);
      data.append("status", "available");
      data.append("temperament", temperament.join(", "));
      data.append("story", introduction);
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.message || "Failed to publish profile.");
      }
      ...
      console.error(err);
      setErrors({
        submit: err.message || "Failed to publish profile. Please try again.",
      });
    } finally {
        ...
        <Card className="max-w-md w-full bg-white dark:bg-gray-800 shadow-xl border border-gray-100 dark:border-gray-700 rounded-[32px] p-8 text-center transition-all duration-300 animate-fadeIn">
          <CardBody className="flex flex-col items-center gap-6">
            ...
            <div>
              <Typography
                variant="h3"
                className="text-2xl font-extrabold text-[#111827] dark:text-white mb-2"
              >
              ...
              >
                <strong>{name}</strong> is now listed. Adopters will be able to
                discover this profile and send adoption requests.
              </Typography>
            </div>
            <div className="flex flex-col w-full gap-3 mt-2">
              <Button
                color="teal"
                onClick={() => navigate("/all-cats")}
              ...
              <Button
                ...
                className="w-full py-3.5 rounded-xl font-bold dark:text-gray-400 dark:hover:bg-gray-700/35"
              >
                Add Another Cat
              </Button>
            </div>
    ...
    <div className="min-h-screen bg-[#f8f9fa] dark:bg-gray-900 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-300">
      <form onSubmit={handleSubmit} className="max-w-4xl w-full">
        ...
        <div className="mb-8">
          <div
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-gray-500 dark:text-gray-400 font-bold text-xs uppercase tracking-widest mb-4 hover:text-gray-800 dark:hover:text-gray-200 cursor-pointer w-fit transition-colors"
          >
          ...
          <Typography
            variant="h2"
            className="text-4xl font-extrabold text-[#111827] dark:text-white mb-2"
          >
            Add New Cat
          </Typography>
          <Typography
            variant="paragraph"
            className="text-gray-500 dark:text-gray-400 text-base max-w-xl"
          >
            Expand our family by adding a new feline friend. Fill out the
            editorial profile below to help them find their soulmate.
          </Typography>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            <Typography
              variant="small"
              className="font-bold text-gray-400 dark:text-gray-500 tracking-widest uppercase text-xs"
            >
              Make Memories !
            </Typography>

            <label
              className={`w-full aspect-[4/5] rounded-[32px] border-2 border-dashed bg-[#f1f5f9] dark:bg-gray-800/50 flex flex-col items-center justify-center text-center p-6 cursor-pointer hover:border-teal-500 dark:hover:border-teal-400 transition-colors overflow-hidden relative group ${errors.mainImage ? "border-red-500 dark:border-red-500/50" : "border-gray-300 dark:border-gray-700"}`}
            >
              <input
                type="file"
                accept="image/*"
              ...
              {mainImage ? (
                ...
                <>
                  <FiCamera
                    className={`w-8 h-8 mb-4 ${errors.mainImage ? "text-red-500 dark:text-red-400 animate-pulse" : "text-teal-600 dark:text-teal-400"}`}
                  />
                  <Typography className="font-bold text-gray-800 dark:text-gray-200 mb-1">
                    Drop high-res portrait here
                  </Typography>
                  <Typography
                    variant="small"
                    className="text-gray-400 dark:text-gray-500 text-xs px-4"
                  >
            ...
            <div className="grid grid-cols-3 gap-4">
              {[0, 1, 2].map((index) => (
                <label
                  key={index}
                  className="aspect-square rounded-2xl bg-[#e2e8f0] dark:bg-gray-800/60 flex items-center justify-center cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-750 transition-colors overflow-hidden relative group"
                >
                  <input
                    type="file"
                    accept="image/*"
              ...
              <CardBody className="p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                  <div>
                    ...
                    >
                      <option className="bg-white dark:bg-gray-850">
                        Adult (3-5 years)
                      </option>
                      <option className="bg-white dark:bg-gray-850">
                        Kitten (0-1 year)
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.introduction)
                        setErrors((prev) => ({ ...prev, introduction: null }));
                    }}
              ...
              <CardBody className="p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                  <div className="flex items-center justify-between bg-white dark:bg-gray-900 rounded-xl px-4 py-3 shadow-sm border border-gray-100 dark:border-gray-850 transition-colors duration-300">
                    <div>
                      <Typography className="font-bold text-gray-800 dark:text-gray-200 text-sm">
                        Vaccinated
                      </Typography>
                      <Typography
                        variant="small"
